...
class UserPredictor:
    ...
    def fit(self, train_users, train_logs, train_y):
        ...
        rfe.fit(train_data[feat], train_data['y'])
        selected_features = rfe.support_

        # Create features from selected features
        selected_features = [feat[i] for i in range(len(feat)) if selected_features[i]]
        ...
        # Train the model
        pipe.fit(train_data[selected_features], train_data['y'])
        self.model = pipe 
        
        # Cross-validation
        scores = cross_val_score(self.model, train_data[selected_features], train_data["y"], cv=5) 
    ...
    def predict(self, test_users, test_logs):
        ...
        # Make predictions
        return self.model.predict(test_data[self.selected_features])